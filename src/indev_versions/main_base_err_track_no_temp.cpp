...
void print_usage(const char *progname) {
  std::cout << "Usage " << progname
            << " [-w value] [-d] group bus device function offset\n\n";
...
int main(int argc, char *argv[]) {
  ...
  uint32 pre_a_err_r1_val = 0, pre_b_err_r1_val = 0, pre_c_err_r1_val = 0,
         pre_d_err_r1_val = 0;
  uint32 pre_a_err_r0_val = 0, pre_b_err_r0_val = 0, pre_c_err_r0_val = 0,
         pre_d_err_r0_val = 0;

  bool write = false;
  bool dec = false;
  bool err_det_r1 = false, err_det_r0 = false;
  bool pre_a_err_det_r1 = false, pre_a_err_det_r0 = false,
       pre_b_err_det_r1 = false, pre_b_err_det_r0 = false,
       pre_c_err_det_r1 = false, pre_c_err_det_r0 = false,
       pre_d_err_det_r1 = false, pre_d_err_det_r0 = false;

  int channel = 0;  // Channel A=0, B=1, C=2, D=3

  int ch_a_group = CH_A_Group, ch_b_group = CH_B_Group, ch_c_group = CH_C_Group,
      ch_d_group = CH_D_Group;
  int ch_a_bus = CH_A_Bus, ch_b_bus = CH_B_Bus, ch_c_bus = CH_C_Bus,
      ch_d_bus = CH_D_Bus;
  int ch_a_device = CH_A_Device, ch_b_device = CH_B_Device,
      ch_c_device = CH_C_Device, ch_d_device = CH_D_Device;
  int ch_a_func = CH_A_Func, ch_b_func = CH_B_Func, ch_c_func = CH_C_Func,
      ch_d_func = CH_D_Func;
  int ch_a_err_func = CH_A_Err_Func, ch_b_err_func = CH_B_Err_Func,
      ch_c_err_func = CH_C_Err_Func, ch_d_err_func = CH_D_Err_Func;
  ...
  if (!drv.start()) {
    ...
    tcerr << "You must have a signed  driver at " << drv.driverPath()
  ...
  try {
    ...
    while (1) {
      ...
      std::cout << " Rank 1 overflow : " << ch_r1_ovrflw
                << " , Rank 0 overflow : " << ch_r0_ovrflw << "\n";
      std::cout << " Rank 1 err count : " << ch_err_r1_val
                << " , Rank 0 err count : " << ch_err_r1_val << "\n\n";
      ...
      if (((ch_r1_ovrflw + ch_r0_ovrflw) == 0) &
          (pre_err_r1_val >= ch_err_r1_val) &
          (pre_err_r0_val >= ch_err_r0_val)) {  // if no error
        if ((err_det_r1 == false) &
            (err_det_r0 == false)) {    // if no error, increase trefI
        ...
        if (ch_r1_ovrflw || (pre_err_r1_val < ch_err_r1_val)) err_det_r1 = true;
        if (ch_r0_ovrflw || (pre_err_r0_val < ch_err_r0_val)) err_det_r0 = true;
        ch_trefi_val = ch_trefi_val - step_tREFI_dec << 1;
        ch_thermal.write32(tREFI_off, ch_tref_const + ch_trefi_val);
      }
